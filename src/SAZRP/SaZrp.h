//
// Main header of the SAZRP (Stability-Aware ZRP) implementation.
//
...
// Self-message types
enum SaZrpSelfMsgType {
    SAZRP_SELF_NDP_HELLO = 0,   // Fixed timer
    SAZRP_SELF_IARP_UPDATE = 1, // Fixed timer
    SAZRP_SELF_DEBUG = 2,       // Fixed timer
    SAZRP_SELF_BRP_JITTER = 10, // Variable: BRP jitter delivery to IERP (carries BRP packet data)
    SAZRP_SELF_IERP_RETRY = 11, // Variable: IERP route request retry timer
};
...
class INET_API SaZrp : public RoutingProtocolBase,
  ...
  protected:
    ...
    std::map<L3Address, double> neighbourStability;                         // neighbour -> smoothed sbar
    std::map<L3Address, std::pair<Coord, Coord>> neighbourKinematics;       // neighbour -> (position, velocity) last known
