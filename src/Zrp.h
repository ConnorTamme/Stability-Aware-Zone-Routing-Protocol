...
class INET_API Zrp : public RoutingProtocolBase,
  ...
  public:
    ...
    // Route quality signals (mirror of SAZRP). Both fire only at the IERP
    // query source on the first arriving reply, so per-module count = number
    // of successful discoveries. Pooling sum/count across all routing modules
    // yields the network-wide average for routes that were actually used.
